//
// Simple passthrough fragment shader with cylinder effect
//
...
void main(void) {
    ...
    float l = length(pos); // distance from position
    if (l > lensSize) {      // if it's outside the distance render as normal
        ...
        // Calculate the cylinder mapping
        float r = atan(n, z) / PI;
        float phi = atan(y, x);

        // Only apply the effect to the top and bottom of the screen
